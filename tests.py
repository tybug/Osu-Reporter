...
class TestMethods(unittest.TestCase):
    def test_calcuate_accuracy_from_play(self):
        play = {
            ...
            "countkatu": "17",
            "countgeki": "174",
        }  # taken from my current top play
    ...
    def test_parse_title_data_from_title(self):
        self.assertEqual(
            parser.parse_title_data("[osu!std] tybug2 | blatant multiaccount"),
            ["0", "tybug2", ["multi", "true"], ["Blatant", "blatant"]],
        )
        self.assertEqual(
            parser.parse_title_data("[osu!taiko]tybug2|spin hacker"),
            ["1", "tybug2", ["spinhack", "false"], ["Cheating", "cheating"]],
        )
        self.assertEqual(
            parser.parse_title_data("[osu!m] not nathan"),
            ["3", "not nathan", ["other", "false"], ["Cheating", "cheating"]],
        )
        self.assertEqual(
            parser.parse_title_data("cookiezi | being too good at the game"), None
        )
        self.assertEqual(parser.parse_title_data("[ sony ]"), None)
        self.assertEqual(
            parser.parse_title_data("[o!ctb][ sony ]"),
            ["2", "[ sony ]", ["other", "false"], ["Cheating", "cheating"]],
        )
        ...
        self.assertEqual(
            parser.parse_title_data("[mania] suki | cheating"),
            ["3", "suki", ["other", "false"], ["Cheating", "cheating"]],
        )
        self.assertEqual(
            parser.parse_title_data("[osu!std] SwordArtOnline_old | cheating"),
            ["0", "SwordArtOnline_old", ["other", "false"], ["Cheating", "cheating"]],
        )
